                ...
                <tbody>
                    @foreach ($files as $file)
                        <tr>
                            ...
                            <td>
                                <div id='qr'>{!! DNS2D::getBarcodeHTML($file->file_number, 'QRCODE', 2, 2) !!}</div>
                            </td>
                            <td>{{ $file->subject }}</td>
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        @if ($file->mode->name == 'generated')
                                            <a class="dropdown-item"
                                                href="{{ route('filetrack.file-generate.edit', Crypt::encrypt($file->id)) }}">
                                                <i class="fa fa-pencil-square-o text-warning"></i> Edit</a>
                                        @endif
                                        <a class="dropdown-item"
                                            href="{{ route('filetrack.file-generate.show', Crypt::encrypt($file->id)) }}">
                                            <i class="fa fa-eye text-primary"></i> View</a>
                                        @if ($file->mode->name == 'generated')
                                            <a class="dropdown-item" href="#"> <i class="fa fa-trash text-danger"></i>
                                                Delete</a>
                                        @endif
                                        @if (
                                            $file->current_user == Auth::guard('fileuser')->user()->id ||
                                                Auth::guard('fileuser')->user()->hasPermissionTo('All File_edit'))
                                            <a class="dropdown-item transfer" href="#"
                                                data-index='{{ $file->id }}'> <i
                                                    class="fa fa-share-square-o text-info"></i>Transfer</a>
                                        @endif
                                    </div>
                                </div>
                            </td>
