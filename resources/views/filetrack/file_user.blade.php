...
@section('head-area')
    ...
    <link rel="stylesheet" type="text/css"
        href="{{ asset('backend/assets/css/plugins/forms/pickers/form-flat-pickr.css') }}">
            ...
            <div class="card-body">
                <form class="needs-validation"
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-1">
                            ...
                            <select name="depoff" id="depoff" class="select2 form-control">
                                <option value="{{isset($editemployee) ? $editemployee->off_dep_id : ''}}">
                                    {{isset($editemployee) ? $editemployee->OfficeDep->name : '--Select Department Or Office--'}}
                                </option>
                        ...
                        <div class="col-md-4 mb-1">
                            ...
                            <select name="branch" id="branch" class="select2 form-control">
                                <option value="{{isset($editemployee) ? $editemployee->off_dep_id : ''}}">
                                    {{isset($editemployee) ? $editemployee->OfficeDep->name : '--Select Branch--'}}</option>
                            ...
                            <select class="form-control select2 form-select" id="select2-basic" name='roleid' required>
                                @if(isset($editemployee))
                                    <option selected hidden value='{{$editemployee->roles[0]->id ?? ''}}'>
                                        {{$editemployee->roles[0]->name ?? ''}}</option>
                                @else
                    ...
                    <div class="row">
                        ...
                        @if (isset($editemployee))
                            <div class="col-sm-6">
                                <img src="{{$editemployee->pic_url }}" class="bg-light-info" alt=""
                ...
                <tbody>
                    @foreach ($employees as $employee)
                        <tr>
                            <td>{{ $loop->index + 1 }}</td>
                            <td>
                                <img src="{{ $employee->pic_url }}" class="me-75 bg-light-danger"
                        ...
                        <form id="delete-form-{{ $eid }}" action="{{ route('filetrack.fileuser.destroy', $eid) }}" method="post"
                            style="display: none;">
                            @method('DELETE')
                            @csrf
                        </form>
